...
This signer implements

* a subset of the functions in the `sign_encrypt.py` script
...
* off-line signing as described in the OP-TEE documentation. The client should
  provide `.dig` (base64-encoded) digests, and (optionally) `.ta-version`
  and `.uuid` files.
...
Example client using digests:

First, the client patches the `sign_encrypt.py`  to create the files needed for off-line signing, similar
...
+    # And save digest, uuid, and ta-version to allow for
+    # off-line signing without having to make drastic
+    # changes to the build system
+    with open(outf_base + '.dig', 'wb+') as digfile:
...
```
Once the compilation is finished, the client can collect up all the `.dig`, `.uuid`, and `.ta-version`
files into a tarball and send them to the signer:
```bash
    find ta -type f -name '*.dig' | sort | xargs tar -c -z -f ${B}/tasign-in.tar.gz
    find ta -type f -name '*.ta' -delete
    digsig_post sign/optee -F "machine=${DIGSIG_MACHINE}" -F"artifact=@${B}/tasign-in.tar.gz" --output ${B}/tasign-out.tar.gz
```
The client can then unpack the resulting `.sig` files and use the `sign_encrypt.py stitch` command to attach
the signature:
```bash
stitch_ta() {
        taname="${1%%.sig}"
        python3 /path/to/enc_sign.py stitch --key ${TA_PUBLIC_KEY} --uuid $(cat ${taname}.uuid) --in ${taname}.stripped.elf --ta-version $(cat ${taname}.ta-version) --sig "$1" --out ${taname}.ta
}

tar -x --no-same-owner --no-same-permissions -f ${B}/tasign-out.tar.gz
find ta -type f -name '*.sig' > ${B}/sigfiles.list
while read sigfile; do
    stitch_ta "$sigfile"
done < ${B}/sigfiles.list
```
